...
$title = 'Nginxキャッシュクリア';
$description = 'baserCMSのサーバキャッシュ削除とは別口で動作する、Nginx専用・キャッシュクリアプラグイン。- 事前に /etc/nginx/nginx.conf 内の ( proxy_cache_pass ~ ) へ記述されたキャッシュディレクトリ名の絶対パス ( /var/www/html 等のスラッシュから始まるパスの事。) を控えておき、Nginxキャッシュディレクトリ更新管理にて更新します。- ※但し、ルートディレクトリ( /var/cache/ , /var/www/ )及び、baserCMSキャッシュディレクトリ( app/tmp/cache/ )をセキュリティ的な観点から固定化しましたので、キャッシュが生成・一時保存されるディレクトリを入力して下さい。)';
